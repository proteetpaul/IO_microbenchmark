...
# numactl --cpunodebind=0 --membind=0 sudo /usr/sbin/biosnoop-bpfcc -Q > biosnoop.txt 2>&1 &
# BIO_PID=$!
...
numactl --cpunodebind=0 --membind=0 ./build/io_uring_microbenchmark \
  --engine "$ENGINE" \
...
sudo /usr/sbin/funclatency-bpfcc io_issue_sqe --microseconds --pid $MICROBENCH_PID --duration 20 &
FUNC_LATENCY_PID2=$!
